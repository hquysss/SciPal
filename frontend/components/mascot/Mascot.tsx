...
export type MascotProps = {
  ...
  ariaLabel?: string;
  /** Optional callback triggered on boop / click */
...
export function Mascot(props: MascotProps) {
  ...
  const boop = () => {
    ...
    let chosenReaction: Reaction = 'heart';
    if (boops.count >= DIZZY_AFTER) {
      boops.count = 0;
      chosenReaction = 'dizzy';
      setReaction('dizzy');
      later(DIZZY_END, null);
    } else {
      setReaction('blink');
      chosenReaction = PAYOFFS[(boops.count - 1) % PAYOFFS.length];
      later(BOOP_PAYOFF, chosenReaction);
      later(BOOP_END, null);
    }

    onBoop?.(chosenReaction);
  ...
  return (
    <button
      ...
      style={{
        ...
        display: 'block',
        flexShrink: 0,
        width: 'var(--mascot-size, ' + size + 'px)',
        height: 'var(--mascot-size, ' + size + 'px)',
        padding: 0,
        border: 0,
