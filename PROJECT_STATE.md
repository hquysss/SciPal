...
4. Ngoài phạm vi giao diện (còn lại sau khi giao diện theo cấp học hoàn tất): quiz bài học thật (API ẩn đáp án, chấm server), nối thẻ thuật ngữ/tài liệu với dữ liệu thật, API AI Tutor, chức năng giao bài cho lớp (nút giả đã bỏ). Nghiệm thu giao diện giáo viên/admin, hồ sơ, tiến trình, phòng thi ở chế độ sáng và tối với tài khoản thật.
...
## Recent Decisions
- **26/09 — Giao diện theo cấp học hoàn tất, dark mode bật (giai đoạn 5):** Studio soạn bài, hàng chờ duyệt, lớp học, quản lý tài khoản dùng token và song ngữ (`PageBreadcrumb`, `Bi`, `Table`, `Dialog`, `LessonStatusBadge`); xem trước trong studio là tờ vở theo cấp của bài. `frontend/theme-baseline.json` rỗng và có test giữ rỗng. `--gate-*`, `--landing-*` và màu dự phòng của lưới môn trỏ về token (xong nợ giai đoạn 2). `DARK_MODE_ENABLED = true`: nút sáng/tối ở navbar (mobile: trong menu), Profile, đăng nhập; Monaco theo chế độ. Quét tương phản tự động trên các trang công khai và trang dựng tạm `/dev/teacher-showcase` ở tối/sáng/theo hệ thống, 4 cấp, 375px — không còn lỗi thật. Bỏ giao diện giả: nút "giao bài tập" báo thành công không có API, nhãn "Hoạt động" cố định, số bài mặc định 4, và nhánh lỗi mạng giả vào lớp demo của `JoinClassModal`. Chưa kiểm với tài khoản thật (không có Supabase ở môi trường cloud).
