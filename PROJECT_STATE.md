...
## Recent Decisions
- **26/09 — Giao diện theo cấp học toàn app (giai đoạn 0–1):** Token theo cấp (Tiểu học hộp sáp màu — hành động hồng; THCS xanh điện; THPT xanh ngọc lục bảo; trung tính) sinh từ `packages/ui/src/theme/palettes.ts`, gắn trên `[data-app-shell]`; script boot chống nháy; navbar cùng màu giấy của trang (token `nav` = `paper`), bỏ hoạ tiết cỏ bốn lá; hoạ tiết đồ dùng học tập mẫu; bỏ Inter; `--accent` không còn trên `:root`. Dark mode có sẵn nhưng tắt bằng `DARK_MODE_ENABLED` tới hết giai đoạn 5. Primitive `components/ui` viết lại cho Tailwind 3 (bản cũ dùng cú pháp v4). Test ratchet màu thô với `frontend/theme-baseline.json`. Theo yêu cầu người dùng, bỏ mực tím và đổi sang bảng màu rực rỡ; landing theo cấp dùng luôn token chung, navbar/menu hết màu xanh lá cứng. QA local `/dev/theme`: 8 section ở 1280px và 375px, không cuộn ngang trang; `/glossary` đổi navbar đổi theo cấp khách và không có cảnh báo hydration. Chưa kiểm được đồng bộ cấp của tài khoản và phòng thi vì môi trường local không có Supabase.
