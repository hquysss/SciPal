...
4. Giai đoạn 5 của spec giao diện theo cấp học (luồng học → cá nhân → giáo viên/admin; bật `DARK_MODE_ENABLED` cuối giai đoạn 5). Ngoài phạm vi giao diện: quiz bài học thật (API ẩn đáp án, chấm server), nối thẻ thuật ngữ/tài liệu với dữ liệu thật, API AI Tutor.
...
## Recent Decisions
- **26/09 — Làm lại cổng chọn cấp và landing (spec `docs/superpowers/specs/2026-09-26-landing-redesign-design.md`, PR hquysss/SciPal#9):** Mục tiêu thu hút người mới, bớt chữ. Giữ luồng chọn cấp trước. Cổng: một dòng tiêu đề + ba cuốn vở CSS 3D; `--gate-*` đã trỏ về token chung (trả xong nợ giai đoạn 2). **Cả ba cấp đã ra mắt** — bỏ nhãn "Sắp ra mắt"/"Sẵn sàng" ở cổng và khung "đang chuẩn bị" ở landing; "Đang biên soạn" chỉ hiện ở môn/bài (thay quyết định "hai cấp nhỏ hiển thị Sắp ra mắt" bên dưới). Landing: hero có cảnh bàn học WebGL (`three`, chunk lười ~137 KB gzip, SVG dự phòng), "Học thế nào" ba thẻ tương tác, giữ gia sư AI (trang Tutor sẽ làm sau; `TutorSection` nhận `href`), khảo sát một dòng cho mọi cấp. Công tắc ngôn ngữ đổi thành cờ 🇻🇳/🇬🇧 (SVG). QA Playwright (`frontend/scripts/qa-landing.mjs`): 1280/375 px × VI/EN × 3 cấp, không cuộn ngang, không cảnh báo hydration, chế độ giảm chuyển động không có canvas, bàn phím chọn được. First Load `/` 126 → 122 kB. Chưa kiểm với tài khoản thật và dữ liệu Supabase (local không có Supabase nên lưới môn hiện lỗi tải).
- **26/09 — Giao diện theo cấp học, giai đoạn 4 (khu vực cá nhân):** Hồ sơ, tiến trình, thi thử và khảo sát dùng token, song ngữ, vùng chạm 44px (16 file về 0 màu thô). Heatmap chuỗi ngày 4 mức `action`, số trong ô là số môn có chuỗi phủ ngày đó (dữ liệu chỉ có `last_active` + `current_streak`, chưa có số bài theo ngày). Đồng hồ thi 3 mức kèm chữ cảnh báo; bảng câu hỏi 3 trạng thái có chú giải; lựa chọn đáp án là radio thật. Khảo sát sau bài báo lỗi khi gửi thất bại (trước đó vẫn cảm ơn). Truy vấn tiến trình lấy thêm cột `_en` để tên bài/môn/huy hiệu theo ngôn ngữ. Chưa kiểm được `/profile`, `/progress`, khảo sát sau bài với tài khoản thật (cần đăng nhập) — đã kiểm qua trang dựng tạm.
...
- **26/09 — Giao diện theo cấp học, giai đoạn 2 (trừ cổng chọn cấp):** Login giữ bố cục Katha nhưng dùng token cấp học (slide minh hoạ hai tông theo `nav`); bỏ `LoginThemeToggle` vì nó gắn `data-theme`/`.dark` lên `<html>` khiến cả app chuyển tối với máy để chế độ tối — test `frontend/lib/theme/rootTheme.test.ts` chặn tái phạm. 404, thông báo lỗi tải, loading, phần sót của navbar dùng token và song ngữ. Bốn bộ hoạ tiết đồ dùng học tập theo cấp trong `frontend/public/patterns/`. Xoá code chết `KathaMascot`, `KhmerMotifs`, `KhmerVine`, `AuthCard`, `ScienceShowcase`. **Còn nợ của giai đoạn 2:** bảng `--gate-*` của cổng chọn cấp (`frontend/features/landing/level-gate.module.css`) vẫn là màu viết cứng, chưa trỏ về token chung.
- **26/09 — Giao diện theo cấp học toàn app (giai đoạn 0–1):** Token theo cấp (Tiểu học nâu nhạt, THCS xanh dương, THPT xanh lá, trung tính) sinh từ `packages/ui/src/theme/palettes.ts`, gắn trên `[data-app-shell]`; script boot chống nháy; navbar tô màu chính của cấp (token `nav`), bỏ hoạ tiết cỏ bốn lá; hoạ tiết đồ dùng học tập mẫu; bỏ Inter; `--accent` không còn trên `:root`. Dark mode có sẵn nhưng tắt bằng `DARK_MODE_ENABLED` tới hết giai đoạn 5. Primitive `components/ui` viết lại cho Tailwind 3 (bản cũ dùng cú pháp v4). Test ratchet màu thô với `frontend/theme-baseline.json`. Theo yêu cầu người dùng: bỏ mực tím; Tiểu học nâu nhạt, THCS xanh dương, THPT xanh lá, navbar theo màu chính của cấp; landing theo cấp dùng luôn token chung, navbar/menu hết màu viết cứng. QA local `/dev/theme`: 8 section ở 1280px và 375px, không cuộn ngang trang; `/glossary` đổi navbar đổi theo cấp khách và không có cảnh báo hydration. Chưa kiểm được đồng bộ cấp của tài khoản và phòng thi vì môi trường local không có Supabase.
...
- `ResourceRefCard` vẫn trỏ cứng tới visualgo.net và `TermRefCard` chưa đọc bảng `terms` — khối `resource-ref`/`term-ref` chưa hiển thị dữ liệu thật.
...
- Một số nội dung giao diện chưa đổi theo ngôn ngữ EN.
